...
<template>
  <div v-editable="blok" class="relative container mb-16 sm:mb-24 lg:mb-32 xl:mb-44">
    ...
    <div class="h-[215px] md:h-[350px] lg:h-[500px] xl:h-[570px] 2xl:h-max w-full hover:cursor-pointer" @click="open">
        <video class="w-full h-full md:object-cover" ref="video" v-if="blok?.src" :src="blok.src.filename + '#t=0.001'" preload="metadata" type='video/mp4' :poster="blok.posterImage.filename" ></video>
    </div>
  </div>
</template>